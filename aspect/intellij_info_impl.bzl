...
def _generate_aar(ctx, file_name, manifest, res_files, resource_root):
    """Generate an AAR with the given input files. This AAR used to be called a "fake" AAR because it doesn't actually exist in sources; rather it's generated for IDE usage purposes. """
...
def _collect_android_ide_info(target, ctx, semantics, ide_info, ide_info_file, output_groups):
    ...
    if hasattr(ctx.rule.attr, "resource_files"):
        ...
        if not (res_artifacts) and android.manifest:
            # The IDE will not add a res folder with an empty relative path, but
            # will add the attached AAR.
            root = struct_omit_none(
                relative_path = "",
                is_source = True,
                is_external = False,
            ...
            aar = _generate_aar(ctx, aar_file_name, android.manifest, [], "")
        ...
        for artifact_path_fragments, res_files in res_artifacts.items():
            ...
            aar = _generate_aar(ctx, aar_file_name, android.manifest, res_files, aar_resource_root)
            resolve_files.append(aar)
