...
def _collect_android_ide_info(target, ctx, semantics, ide_info, ide_info_file, output_groups):
    ...
    if hasattr(ctx.rule.attr, "resource_files"):
        ...
        if not (res_artifacts) and android.manifest:
            ...
            # We use a non-existent res folder here because there isn't a res folder to
            # point to and we can't use the target's containing directory as a res folder.
            # The res folder root is irrelevant in the case of manifest-only AARs anyway.
            root = to_artifact_location(target.label.package + "-dummy-res-folder", None, True, False)
