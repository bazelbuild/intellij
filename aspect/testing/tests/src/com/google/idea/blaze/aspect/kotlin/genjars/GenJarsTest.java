 ...
 */
package com.google.idea.blaze.aspect.kotlin.genjars;
...
/** Tests generated jars from Kotlin libraries. */
...
public class GenJarsTest extends BazelIntellijAspectTest {
  ...
  @Test
  public void testKotlinLibraryWithGeneratedSourcesHasGenJars() throws Exception {
    ...
    assertThat(
            targetIdeInfo.getJavaIdeInfo().getGeneratedJarsList().stream()
                .map(IntellijAspectTest::libraryArtifactToString)
                .collect(toList()))
        .containsExactly(
            jarString(
                testRelative("has_plugin-java.jar"), null, testRelative("has_plugin-kapt.srcjar")));

    assertThat(getOutputGroupFiles(testFixture, "intellij-info-java"))
        .contains(testRelative(intellijInfoFileName("has_plugin")));
    assertThat(getOutputGroupFiles(testFixture, "intellij-resolve-java"))
        .containsAllOf(
            testRelative("libhas_plugin.jar"),
            testRelative("libhas_plugin-src.jar"),
            testRelative("has_plugin-java.jar"),
            testRelative("has_plugin-kapt.srcjar"));
    assertThat(getOutputGroupFiles(testFixture, "intellij-compile-java"))
        .containsAllOf(testRelative("libhas_plugin.jar"), testRelative("has_plugin-java.jar"));
