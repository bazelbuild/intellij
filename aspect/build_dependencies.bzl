...
def _package_dependencies_impl(target, ctx):
    ...
    return [OutputGroupInfo(
        ...
        qsync_ext_gensrcs = target[DependenciesInfo].ext_gensrcs.to_list(),
    )]
...
DependenciesInfo = provider(
    ...
    fields = {
        ...
        "ext_gensrcs": "a list of sources generated by external dependencies",
    ...
    for label, target_info in target[DependenciesInfo].target_to_artifacts.items():
        contents.append(
            struct(
                ...
                ext_gen_srcs = target_info["ext_gen_srcs"],
...
def _collect_own_and_dependency_artifacts(
    ...
    if has_own_artifacts:
        ...
        target_to_artifacts[str(target.label)] = {
            ...
            "gen_srcs": [_output_relative_path(file.path) for file in gen_srcs] if target_is_within_project_scope else [],
            "ext_gen_srcs": [_output_relative_path(file.path) for file in gen_srcs] if not target_is_within_project_scope else [],
            "srcs": own_src_files,
            "srcjars": own_srcjar_files,
        }
        #print(target.label, "gen_srcs", target_to_artifacts[str(target.label)]["gen_srcs"])
    ...
    own_and_transitive_ext_gensrc_depsets = []
    ...
    for info in dependency_infos:
        ...
        own_and_transitive_ext_gensrc_depsets.append(info.ext_gensrcs)
    ...
    return (
        ...
        depset(own_gensrc_files if target_is_within_project_scope else [], transitive = own_and_transitive_gensrc_depsets),
        depset(own_gensrc_files if not target_is_within_project_scope else [], transitive = own_and_transitive_ext_gensrc_depsets),
    )
...
def _collect_dependencies_core_impl(
    ...
    target_to_artifacts, compile_jars, aars, gensrcs, ext_gensrcs = _collect_own_and_dependency_artifacts(
        target,
        ctx,
        dependency_infos,
        always_build_rules,
    ...
    if test_mode:
        (
            ...
            within_scope_own_ext_getsrc_files,
            _,
        ...
        test_mode_own_files = struct(
            ...
            test_mode_within_scope_own_ext_getsrc_files = within_scope_own_ext_getsrc_files,
        )
