...
def _collect_dependencies_impl(target, ctx):
    return __collect_dependencies_impl(
        target,
        ctx,
        ctx.attr.include,
        ctx.attr.exclude,
...
def __collect_dependencies_impl(
        target,
        ctx,
        include = "",
        exclude = "",
        always_build_rules = "",
        generate_aidl_classes = False):
    label = str(target.label)
    included = False
    if include == "":
        # include can only be empty only when used from collect_all_dependencies
...
collect_all_dependencies = aspect(
    doc = """
    ...
    """,
    implementation = __collect_dependencies_impl,
    provides = [DependenciesInfo],
    attr_aspects = ["deps", "exports", "_junit"],
    required_providers = [[JavaInfo]],
)
