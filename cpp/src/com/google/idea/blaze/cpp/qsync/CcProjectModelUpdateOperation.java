...
public class CcProjectModelUpdateOperation {
  ...
  private CLanguageKind getLanguageKind(CcLanguage language, String whatFor) {
    CLanguageKind kind = LANGUAGE_MAP.get(language);
    if (kind == null) {
      logger.warn("Unknown language for " + whatFor + ": " + language);
    }
    return kind;
  }
  ...
  /** Pre-commits the project update. Must be called from the write thread. */
  public void commit() {
    WriteAction.compute(modifiableOcWorkspace::commit);
  }
