...
import com.google.idea.blaze.base.sync.SimpleSyncListener;
...
class BlazeTypeScriptConfigServiceImpl implements TypeScriptConfigService {
  ...
  static class SyncListener implements SimpleSyncListener {
    @Override
    public void syncFinished(Project project, SyncMode syncMode, SyncResult syncResult) {
      TypeScriptConfigService service = TypeScriptConfigService.Provider.get(project);
      if (service instanceof BlazeTypeScriptConfigService) {
        ((BlazeTypeScriptConfigService) service).update();
      }
    }
  }
}
