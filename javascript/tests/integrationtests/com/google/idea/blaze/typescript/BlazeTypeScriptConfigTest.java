...
public class BlazeTypeScriptConfigTest extends BlazeIntegrationTestCase {
  ...
  public final void before() {
    ...
    this.regularConfigService =
        new TypeScriptConfigServiceImpl(
            getProject(), null, new TypeScriptConfigGraphCache(getProject()));
  }

  @Test
  public void testCompileOnsave() {
    TypeScriptConfig blazeConfig = blazeConfigService.getConfigFiles().get(0);
  ...
  public void testSameOptions() {
    assertThat(blazeConfigService.getConfigFiles()).hasSize(1);
    assertThat(regularConfigService.getConfigFiles()).hasSize(1);
    TypeScriptConfig blazeConfig = blazeConfigService.getConfigFiles().get(0);
    TypeScriptConfig regularConfig = regularConfigService.getConfigFiles().get(0);
    ...
    assertThat(blazeConfig.getLibNames()).isEqualTo(regularConfig.getLibNames());
    assertThat(blazeConfig.getTypeRoots()).isEmpty();
    assertThat(blazeConfig.getTypeRoots()).isEqualTo(regularConfig.getTypeRoots());
    ...
    assertThat(blazeConfig.getTypes()).isEqualTo(regularConfig.getTypes());
    ...
    assertThat(blazeConfig.getIncludePatterns()).isEqualTo(regularConfig.getIncludePatterns());
    assertThat(blazeConfig.getExcludePatterns()).isEqualTo(regularConfig.getExcludePatterns());
    ...
    assertThat(blazeConfig.getProjectReferences()).isEqualTo(regularConfig.getProjectReferences());
  ...
  public void testDifferentOptions() {
    ...
    ImmutableList<JSModulePathSubstitution> paths =
        blazeConfig.getPaths().stream()
            .sorted(Comparator.comparing(JSModulePathSubstitution::getMappedName))
