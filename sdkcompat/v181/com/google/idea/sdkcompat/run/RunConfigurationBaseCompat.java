...
public class RunConfigurationBaseCompat {
  ...
  public static void addBeforeRunTask(RunConfigurationBase config, BeforeRunTask<?> task) {
    ...
    // list while the run configuration is being edited, so we use RunManagerEx.getBeforeRunTasks
    // instead.
    @SuppressWarnings("rawtypes")
    List<BeforeRunTask> tasks = new ArrayList<>(getAllBeforeRunTasks(config));
    tasks.add(task);
