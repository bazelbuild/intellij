...
public class BuildDependenciesHelper {
  ...
  public int getUnbuiltExternalDepsFor(TargetsToBuild toBuild) {
    if (toBuild.type() == TargetsToBuild.Type.BUILD_ALL) {
      // Not a source file, don't show unbuilt deps count.
      return 0;
    }
  ...
  public VirtualFile getVirtualFile(AnActionEvent e) {
  ...
  public void enableAnalysis(AnActionEvent e, PopupPosititioner positioner) {
    ...
    if (!toBuild.isAmbiguous()) {
      QuerySyncManager.getInstance(project).enableAnalysis(toBuild.targets());
    }
    if (toBuild.type() == TargetsToBuild.Type.CHOOSE_ONE && toBuild.targets().size() > 1) {
      chooseTargetToBuildFor(
          vfile, toBuild, positioner, label -> syncManager.enableAnalysis(ImmutableSet.of(label)));
    }
  }
