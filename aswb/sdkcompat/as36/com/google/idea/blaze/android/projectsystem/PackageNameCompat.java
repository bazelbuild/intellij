...
public class PackageNameCompat {
  ...
  private static String doGetPackageName(AndroidFacet facet) {
    ...
    return getPackageNameFromPrimaryManifest(facet);
  }
  ...
  private static String getPackageNameFromPrimaryManifest(AndroidFacet facet) {
    ...
    PsiFile manifestPsi =
        AndroidPsiUtils.getPsiFileSafely(facet.getModule().getProject(), manifest);
    if (!(manifestPsi instanceof XmlFile)) {
      return null;
    }
    return AndroidManifestUtils.getPackageName((XmlFile) manifestPsi);
  }
}
