...
public class BlazeModuleSystemDependentLibrariesIntegrationTest
  ...
  public void setup() {
    ...
    AarLibraryFileBuilder.aar(workspaceRoot, aarTarget.getAar().getRelativePath())
        .src(
            ...
            ImmutableList.of(
                ...
                "</resources>"))
        .build();
    generateArtifactLocation(aarTarget.getAar().getRelativePath());
    ...
    binaryTarget
        ...
        .forEach(
            aar -> {
              AarLibraryFileBuilder.aar(workspaceRoot, aar.getRelativePath())
                  .src(
                      "res/values/values.xml",
                      ImmutableList.of(
                          "<?xml version=\"1.0\" encoding=\"utf-8\"?>",
                          "<resources>",
                          "    <color name=\"binaryValue\">#ffffff</color>",
                          "</resources>"))
                  .build();
              generateArtifactLocation(aar.getRelativePath());
            });
              ...
              AarLibraryFileBuilder.aar(workspaceRoot, aar.getRelativePath())
                  .src(
                      ...
                      ImmutableList.of(
                          "<?xml version=\"1.0\" encoding=\"utf-8\"?>",
                          "<resources>",
                          "    <color name=\"quantumColor\">#ffffff</color>",
                          "</resources>"))
                  .build();
              generateArtifactLocation(aar.getRelativePath());
            });
              ...
              AarLibraryFileBuilder.aar(workspaceRoot, aar.getRelativePath())
                  .src(
                      ...
                      ImmutableList.of(
                          "<?xml version=\"1.0\" encoding=\"utf-8\"?>",
                          "<resources>",
                          "    <color name=\"constraintColor\">#ffffff</color>",
                          "</resources>"))
                  .build();
              generateArtifactLocation(aar.getRelativePath());
            });
  ...
  private ExternalAndroidLibrary getAarLibrary(String aarPath, @Nullable String resourcePackage) {
    ...
    AarLibrary library = AarLibraryFactory.create(source(aarPath), resourcePackage);
    System.out.println("aarPath " + aarPath);
    System.out.println(
        "unpackedAars.getResourceDirectory(library) " + unpackedAars.getResourceDirectory(library));
    PathString resFolder = new PathString(unpackedAars.getResourceDirectory(library));
    PathString aarFile = new PathString(unpackedAars.getAarDir(library));
