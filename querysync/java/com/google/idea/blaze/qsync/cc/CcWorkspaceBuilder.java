...
public class CcWorkspaceBuilder {
  ...
  private static final ImmutableMap<String, CcLanguage> EXTENSION_TO_LANGUAGE_MAP =
      ImmutableMap.of(
          ...
          "c++", CcLanguage.CPP,
          "C", CcLanguage.C);
  // Files we ignore because the are not top level source files:
  ...
  private Optional<CcLanguage> getLanguage(Path srcPath) {
    ...
    if (lastDot < 0) {
      // default to cpp
      context.output(PrintOutput.log("No extension for cc source file %s; assuming cpp", srcPath));
    ...
    context.output(
        PrintOutput.log(
            "Unrecognized extension %s for cc source file %s; assuming cpp", ext, srcPath));
    return Optional.of(CcLanguage.CPP);
  }
}
