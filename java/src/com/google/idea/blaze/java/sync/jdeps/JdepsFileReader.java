...
public class JdepsFileReader {
  ...
  private JdepsState doLoadJdepsFiles(
    ...
    ListenableFuture<?> fetchRemoteFuture =
        RemoteFilePrefetchService.getInstance().prefetchFiles(project, context, outputArtifacts);
    if (!FutureUtil.waitForFuture(context, Futures.allAsList(fetchFuture, fetchRemoteFuture))
