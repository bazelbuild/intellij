...
public final class BlazeKotlinWorkspaceImporter {
  ...
  public BlazeKotlinImportResult importWorkspace() {
    ...
    Collection<Kind> kotlinKinds = Kind.allKindsForLanguage(LanguageClass.KOTLIN);
    List<TargetKey> kotlinSourceTargets =
        targetMap
            .targets()
            .stream()
            .filter(target -> target.javaIdeInfo != null)
            .filter(target -> target.kindIsOneOf(kotlinKinds))
    ...
    for (TargetKey dependency :
        TransitiveDependencyMap.getTransitiveDependencies(kotlinSourceTargets, targetMap)) {
