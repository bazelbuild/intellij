...
_JDK_CLASSES_EXCLUDE = [
    # not provided by the JRE
    # need to come first since everything else in javax is from the JDK
    "javax.annotation.",
]
...
def repackaged_jetbrains_jars(name, jars, **kwargs):
    ...
    # NOTE: entry order matters, more specific transforms must come first
    rules = {c: "_jetbrains_." + c for c in _JDK_CLASSES_EXCLUDE}
    rules.update({c: c for c in _JDK_CLASSES + _JETBRAINS_CLASSES})
    ...
    native.java_import(
        name = name + "_unrepackaged",
        jars = jars,
        visibility = ["//javatests/com/google/devtools/intellij:__subpackages__"],
        **kwargs
    )
...
def repackaged_plugin_jars(name, jars, **kwargs):
    ...
    # NOTE: entry order matters, more specific transforms must come first
    rules = {c: "_google_." + c for c in _JDK_CLASSES_EXCLUDE}
    rules.update({c: c for c in _JDK_CLASSES + _JETBRAINS_CLASSES + _OUR_CLASSES})
